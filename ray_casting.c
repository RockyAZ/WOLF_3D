...
int		vertic_inter(t_main *win, float angle, int pix)
{
...
void	ray_casting(t_main *win)
{
	...
	i = 0;
	angle = win->gg.angle - (FOV / 2);
	glClear(GL_COLOR_BUFFER_BIT/* | GL_DEPTH_BUFFER_BIT*/);
	glBegin(GL_LINES);
	while (i < WIDTH)
	{
		line_draw(win, i, CUBE / ft_min(horiz_inter(win, angle), vertic_inter(win, angle, i) * win->gg.to_screen));
